...
function seedDemoUsers(tenantId: string) {
  ...
  const seeds: Array<Omit<SandboxUser, 'userId'>> = [
    ...
    },
    {
      ...
      displayName: 'Model Risk Validator',
      role: 'analyst',
      status: 'active',
      password: 'sandbox-validator-8',
      createdAt: now,
      updatedAt: now,
    },
    {
    ...
    },
    {
      tenantId,
      email: 'po@demo.local',
      displayName: 'Use-case Product Owner',
      role: 'ops',
      status: 'active',
      password: 'sandbox-po-8chars',
      createdAt: now,
      updatedAt: now,
    },
    {
...
export function generateApiKeySecret(prefix = 'aetherlab_demo'): string {
...
export function getOrCreateTenantProfile(tenantId: string) {
  ...
  return {
    tenantId,
    displayNameEn: 'Aetherlab demo bank',
    displayNameAr: 'Aetherlab',
  };
}
