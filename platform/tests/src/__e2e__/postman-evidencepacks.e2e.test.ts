/**
 * Postman-collection 1:1 Vitest tests for evidencepacks (generated)
 *
...
describe("Postman / evidencepacks (1:1 generated)", () => {

  it("listEvidencePacks", async () => {
    const url = sub("{{baseUrl}}/v1/evidence-packs?cursor={{cursor}}&limit={{limit}}&modelId={{modelId}}");
  ...
  it("createModelEvidencePack", async () => {
    ...
    const res = await fetch(url, {
      ...
      body: sub("{\n  \"modelId\": \"newman_modelId\"\n}"),
    });
    expect(res.status).toBe(201);
    const j = await res.json(); expect(j).toHaveProperty("data");
    if (j?.data?.id) vars['modelEvidencePackId'] = j.data.id;
  });
  ...
  it("exportEvidencePack", async () => {
    const url = sub("{{baseUrl}}/v1/evidence-packs/{{evidencePackId}}/export");
    const res = await fetch(url, {
      method: "POST",
      headers: vars.accessToken ? { Authorization: `Bearer ${vars.accessToken}` } : {},
    });
