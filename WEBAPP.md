...
- **Amazon SageMaker Model Dashboard / Model Registry:** Model cards, approval states, endpoint inventory. Steal: registry-first navigation and stage transitions; reject raw AWS console sprawl as the bank’s examiner UI.
- **MLflow / Databricks Unity Catalog model governance:** Lineage from dataset to run to model version. Steal: clickable lineage graph; reject notebook-first as the only operator home.
- **Collibra / Alation (data governance UX):** Purpose-bound access and exam-ready packs. Steal: evidence pack export as a first-class object (BR-8); reject generic data-catalogue aesthetics for MLOps density.
- **Weights & Biases / Evidently monitoring:** Drift and performance attached to deployments. Steal: monitors-required-or-blocked deploy (BR-5); reject research-lab chart walls without business-goal KPIs (BR-3).
...
Quants bypass friction unless the factory is the fastest compliant path. High-risk classes (credit, AML, trading, adverse action) get stricter gates (BR-10). Factory orchestrates/records—it does not replace domain product APIs (BR-11). Multi-cloud locations still report into one inventory (BR-12). Dataset access is purpose-bound and expiring.
...
| Model risk validator | Evaluations / approval queue | Business-goal sign-off (BR-3, BR-4) |
...
|----------|---------------------------|
| Projects / inventory | Projects, Models |
| Lineage | Datasets |
| Train orchestration records | TrainingJobs |
| Business-goal gates | Evaluations |
| Approvals / endpoints | Deployments |
| Drift / perf / latency / cost | Monitors |
...
- **Primary actions:** Register project/model; open detail; kill zombie train; force-register dark find.
- **Empty / loading / error:** Empty = guided “frame first model”; error = sync failure with cloud registry.
- **BR / story ties:** BR-12; platform head stories.
...
- **Empty / loading / error:** Incomplete KPI blocks GPU scale grant (BR-1).
...
- **Empty / loading / error:** Missing lineage blocks train launch (BR-2).
- **BR / story ties:** BR-2; security admin stories.

### Training jobs

- **Purpose:** Orchestration records for jobs/GPUs—including transfer-learning base inventory.
- **Entry:** From framed model; cost alert deep link.
- **Layout regions:** Job list (status, cost, base weights); model-zoo dependency notes; kill controls; link to frame/lineage.
- **Primary actions:** Launch (if gated); stop zombie; register base-weight licence/risk note.
- **Empty / loading / error:** No frame = CTA to framing; licence missing on zoo base = warn (BR-6).
...
- **Primary actions:** Pass/fail for deploy eligibility; request retrain; attach artefact.
- **Empty / loading / error:** Missing business KPI artefact = fail closed (BR-3).
- **BR / story ties:** BR-3; validator stories.

### Deployment dual-control

- **Purpose:** Production deploy requires builder ≠ validator dual control; monitors attached or blocked.
- **Entry:** Eval passed; CI/CD agent callback.
- **Layout regions:** Endpoint plan; builder/validator slots; monitor attachment checklist; high-risk extra gates; rollback plan.
- **Primary actions:** Request approve; validator sign; deploy; block if monitors missing.
- **Empty / loading / error:** Same-user both slots rejected; missing monitor = hard block (BR-4, BR-5).
...
- **Empty / loading / error:** Unmonitored prod = coral banner inventory-wide (BR-5).
- **BR / story ties:** BR-5; product owner stories.
...
- **Empty / loading / error:** Dependents remaining block retire.
- **BR / story ties:** BR-7.
...
- **Layout regions:** Pack builder; included artefacts checklist; hash/attestation; export history.
- **Primary actions:** Generate pack; download; share read-only link (time-boxed).
- **Empty / loading / error:** Incomplete artefacts listed before export (BR-8).
- **BR / story ties:** BR-8; internal audit stories.

### Cloud spend by model

- **Purpose:** GPU/cloud spend by model and team to curb sprawl.
...
- **Empty / loading / error:** Cost API lag banner.
- **BR / story ties:** BR-9.

### Risk-class gate matrix

- **Purpose:** Stricter gates for credit/AML/trading/adverse-action vs low-risk experiment.
- **Entry:** Admin; shown contextually on framing/deploy.
...
- **Empty / loading / error:** Unclassified model cannot deploy.
- **BR / story ties:** BR-10.
...
2. **Examiner sampling** — pick model id → generate evidence pack → export lineage/approvals/metrics/incidents (BR-8).
...
5. **High-risk exception** — credit/AML/trading class → extra gate checklist → dual exception approve or deny (BR-10).
...
- `--color-cyan-gate: #2EC4B6` — gate pass / approved
- `--color-amber-gap: #E9A820` — missing monitor / incomplete pack
- `--color-scrap: #E4572E` — zombie job / orphan endpoint
...
- Grotesk display for stage names and inventory titles; mono for ids/hashes/cost figures.
...
- Replacement of SageMaker/Databricks IDEs; domain fraud/credit/trading business UIs; public model marketplace; headset/AR factory; unmanaged personal cloud accounts as supported loci.
